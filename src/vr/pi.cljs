...
(defn format-date
  ([date-as-string-in-default-format]
   (format-date date-as-string-in-default-format "yy-mm-dd"))

  ([date-as-string-in-default-format output-python-format]
...
(defn date-from-utc-string [date-as-string-in-default-format]
  (dt/parse default-date-formatter date-as-string-in-default-format))
...
(defn ^:export set-loading [state value]
...
(defn translate [mykey mystring]
  ...
  ;;   => "Name"
  ;; mykey = "af_ZA" => "af_ZA" !!!
  (let [item (->> (clojure.string/split mystring #"\|")
